...
def StartBotSender():
    ...
    for em, to_email1 in zip(from_emails, to_emails):
        if em.find('@yahoo.com') != -1:
            ...
            smtps = 'smtp.mail.ru'
        elif em.find('@ya.ru') != -1 or em.find('@yandex.ru') != -1:
            smtps = 'smtp.yandex.ru'
        elif em.find('@gmail.ru') != -1:
            smtps = 'smtp.gmail.com'
        elif em.find('@rambler.ru') != -1 or em.find('@ro.ru') != -1 or em.find('@myrambler.ru') != -1 or em.find('@autorambler.ru') != -1:
            smtps = 'smtp.rambler.ru'
        else:
